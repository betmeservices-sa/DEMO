        ...
        <div className="flex flex-col gap-1">
          {vivas.map((e, i) => {
            ...
            return (
              ...
              >
                <span className="w-36 shrink-0 truncate text-[11.5px] font-semibold text-[var(--text-2)]">
                  {e.nombre}
                </span>
                <span className="relative h-7 flex-1">
                  ...
                  >
                    ...
                    {e.monto > 0 && <span className="text-[11px] font-semibold tabular-nums">{usdCorto(e.monto)}</span>}
                  </span>
                </span>
                <ChevronRight
                  size={14}
                  className={cn(
                    "shrink-0 text-[var(--text-3)] transition",
                    activo ? "rotate-90 text-brand" : "opacity-0 group-hover:opacity-100",
                  )}
                />
              </button>
            );
