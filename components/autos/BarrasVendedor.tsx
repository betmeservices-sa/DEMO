...
// Cuántos prospectos le tocaron a cada vendedor, y de dónde salieron.
//
// Es el gemelo del de CrediQ, adaptado a la sala de ventas: donde allá dice
// "lead" y "monto de la solicitud", acá dice prospecto, modelo y precio.
//
...
// comparando largos, no leyendo números en fila. La tabla de más abajo contesta
// la otra pregunta, la de quién cierra; son dos preguntas distintas y por eso
// están las dos.
//
// Cada barra va partida por canal, porque la segunda pregunta llega sola: si un
// vendedor entrega el doble que otro, importa saber si le están entrando por
...
import { CANAL, porCanal, type Vendedor } from "@/lib/autos-pipeline";
...
export function BarrasVendedor({ casos, vendedores }: { casos: Venta[]; vendedores: Vendedor[] }) {
  ...
  return (
    <section className="rounded-2xl border border-line bg-card p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-[14px] font-bold text-[var(--text)]">Prospectos asignados por vendedor</h3>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="relative h-7 flex-1 overflow-hidden rounded-lg bg-surface">
                    {/* Un solo bloque de ancho proporcional al vendedor con más
                        prospectos, partido por dentro según el canal. */}
                    <span
                      className={cn(
                        "ccg-barra absolute inset-y-0 left-0 flex overflow-hidden rounded-lg transition-all duration-500 ease-out",
                        activo ? "brightness-110" : "group-hover:brightness-110",
                      )}
                      style={{
                        width: `${Math.max(4, (suyos / tope) * 100)}%`,
                    ...
                    {f.casos.length === 0 ? (
                      ...
                      f.casos.map((c, j) => (
                        <li
                          ...
                          style={{ animationDelay: `${Math.min(j, 12) * 22}ms` }}
                        >
                          <span className="flex min-w-0 items-center gap-2">
                            ...
                            <span className="min-w-0 truncate text-[12.5px] text-[var(--text)]">
                              {c.nombre}
                              <span className="text-[var(--text-3)]"> · {nombreDeModelo(c.modelo)}</span>
                            </span>
                          </span>
                          <span
                            className={cn(
                          ...
                          >
                            {c.monto ? usd(c.monto) : "sin precio"}
                          </span>
                        </li>
                      ))
                    )}
